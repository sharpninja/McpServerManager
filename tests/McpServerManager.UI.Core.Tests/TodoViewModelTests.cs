...
public sealed class TodoViewModelTests
{
    ...
    [Fact]
    public async Task TodoListHostViewModel_SaveNewTodoAsync_ConsecutiveCreatesBothSucceed()
    {
        var apiClient = Substitute.For<ITodoApiClient>();
        apiClient.CreateTodoAsync(Arg.Any<CreateTodoCommand>(), Arg.Any<CancellationToken>())
            .Returns(call =>
        ...
        host.NewTodoTitle = "First TODO";
        await host.SaveNewTodoForTestAsync();
        Assert.DoesNotContain("failed", host.StatusText, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Error", host.StatusText, StringComparison.Ordinal);

        host.NewTodoTitle = "Second TODO";
        await host.SaveNewTodoForTestAsync();
        Assert.DoesNotContain("failed", host.StatusText, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Error", host.StatusText, StringComparison.Ordinal);
    ...
    [Fact]
    public async Task TodoListHostViewModel_SaveEditorAsync_ConsecutiveNewTodosViaEditorBothSucceed()
    {
        var apiClient = Substitute.For<ITodoApiClient>();
        apiClient.CreateTodoAsync(Arg.Any<CreateTodoCommand>(), Arg.Any<CancellationToken>())
            .Returns(call =>
        ...
        await apiClient.Received(2)
            .CreateTodoAsync(Arg.Any<CreateTodoCommand>(), Arg.Any<CancellationToken>());
        await apiClient.Received(2)
            .GetTodoAsync(Arg.Any<string>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task TodoListHostViewModel_MoveSelectedTodoCommand_LoadsTargetsAndMovesThroughCqrs()
    {
        ...
        apiClient.ListTodosAsync(Arg.Any<ListTodosQuery>(), Arg.Any<CancellationToken>())
            .Returns(new ListTodosResult([], 0));
        apiClient.MoveTodoAsync(Arg.Any<MoveTodoCommand>(), Arg.Any<CancellationToken>())
            .Returns(call =>
            {
                var command = call.Arg<MoveTodoCommand>()!;
                return new TodoMutationOutcome(
                    true,
                    null,
                    CreateTodoDetail(command.TodoId));
            });
        ...
        workspaceApi.ListWorkspacesAsync(Arg.Any<CancellationToken>())
            .Returns(new ListWorkspacesResult(
                [
                    new WorkspaceSummary("F:\\GitHub\\Current", "Current", true, true),
                    new WorkspaceSummary("F:\\GitHub\\Target", "Target", false, true),
                ],
                2));

        using var sp = BuildProvider(apiClient, workspaceApi);
        var host = CreateTrackingHost(sp);
        host.ApplyWorkspacePath("F:/GitHub/Current");
        host.SelectedEntry = CreateTodoListEntry("TODO-MOVE-001");

        await host.MoveSelectedForTestAsync();

        Assert.True(host.IsMoveTargetSelectorVisible);
        Assert.Single(host.MoveTargets);
        Assert.Equal("F:\\GitHub\\Target", host.SelectedMoveTarget?.WorkspacePath);

        await host.ConfirmMoveForTestAsync();

        await apiClient.Received(1).MoveTodoAsync(
            Arg.Is<MoveTodoCommand>(command =>
                command.TodoId == "TODO-MOVE-001" &&
                command.TargetWorkspacePath == "F:\\GitHub\\Target"),
            Arg.Any<CancellationToken>());
        Assert.False(host.IsMoveTargetSelectorVisible);
        Assert.Contains("Moved TODO-MOVE-001", host.StatusText, StringComparison.Ordinal);
    }

    private static ServiceProvider BuildProvider(ITodoApiClient apiClient, IWorkspaceApiClient? workspaceApiClient = null)
    {
        ...
        services.AddSingleton(workspaceApiClient ?? Substitute.For<IWorkspaceApiClient>());
    ...
    private static TodoDetail CreateTodoDetail(string id)
        => new(
            id,
            "Sample TODO",
            "general",
    ...
    private sealed class TrackingTodoListHostViewModel : TodoListHostViewModel
    {
        ...
        public Task MoveSelectedForTestAsync() => MoveSelectedTodoAsync();

        public Task ConfirmMoveForTestAsync() => ConfirmMoveTodoAsync();

        public Task SaveNewTodoForTestAsync() => SaveNewTodoAsync();

        public void NewTodoForTest() => NewTodo();

        public Task SaveEditorForTestAsync() => SaveEditorAsync();
    }
