...
public sealed class WebUiPhase5ViewModelTests
{
    ...
    [Fact]
    public void UiCoreMessageMapper_ToSessionLogTurnDetail_PreservesCommits()
    {
        var turn = new UnifiedSessionTurn
        {
            RequestId = "req-commit",
            QueryTitle = "Commit test"
        };
        ...
        turn.Commits[0].FilesChanged.Add("src/file.cs");

        var detail = UiCoreMessageMapper.ToSessionLogTurnDetail(turn);

        Assert.Single(detail.Commits);
        Assert.Equal("abcdef1234567890", detail.Commits[0].Sha);
        Assert.Equal("main", detail.Commits[0].Branch);
        Assert.Equal("Add commits section", detail.Commits[0].Message);
        Assert.Equal("Codex", detail.Commits[0].Author);
        Assert.Equal("2026-05-21T20:00:00Z", detail.Commits[0].Timestamp);
        Assert.Equal(["src/file.cs"], detail.Commits[0].FilesChanged);
    }
