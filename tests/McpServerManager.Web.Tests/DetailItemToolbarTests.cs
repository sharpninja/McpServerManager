...
public sealed class DetailItemToolbarTests
{
    ...
    [Fact]
    public void WorkspaceDetail_SavesPromptOverridesAndPolicyEdits()
    {
        const string currentPath = @"C:\repo-two";
        var detail = new WorkspaceDetail(
            currentPath,
        ...
        UpdateWorkspaceCommand? updateCommand = null;
        UpdateWorkspacePolicyCommand? policyCommand = null;
        var api = new WorkspaceApiClientStub
        {
            OnListWorkspacesAsync = _ => Task.FromResult(new ListWorkspacesResult(
            [
            ...
            OnGetWorkspaceAsync = (_, _) => Task.FromResult<WorkspaceDetail?>(detail),
            OnUpdateWorkspaceAsync = (command, _) =>
            {
                updateCommand = command;
                return Task.FromResult(new WorkspaceMutationOutcome(true, null, detail));
            },
            OnUpdateWorkspacePolicyAsync = (command, _) =>
            {
        ...
        {
            services.AddSingleton<IWorkspaceApiClient>(api);
            services.AddSingleton<IAuthorizationPolicyService>(new AllowAllAuthorizationPolicyService());
        });
        ctx.Services.GetRequiredService<BunitNavigationManager>()
            .NavigateTo($"/workspaces/detail?path={Uri.EscapeDataString(currentPath)}");
        var cut = ctx.Render<McpServerManager.Web.Pages.Workspaces.WorkspaceDetail>();

        cut.WaitForElement("[data-testid='workspace-status-prompt']");
        cut.WaitForAssertion(() =>
            Assert.Equal("Save", cut.Find(".detail-card-expander__actions .btn-primary").TextContent));
        cut.Find("[data-testid='workspace-status-prompt']").Change("updated status");
        cut.Find("[data-testid='workspace-implement-prompt']").Change("updated implement");
        cut.Find("[data-testid='workspace-plan-prompt']").Change("updated plan");
        cut.Find("[data-testid='workspace-banned-licenses']").Change("MIT\nApache-2.0");
        cut.Find("[data-testid='workspace-banned-organizations']").Change("Tailspin");
        cut.Find(".detail-card-expander__actions .btn-primary").Click();

        cut.WaitForAssertion(() =>
        {
            Assert.NotNull(updateCommand);
            Assert.Equal("updated status", updateCommand!.StatusPrompt);
            Assert.Equal("updated implement", updateCommand.ImplementPrompt);
            Assert.Equal("updated plan", updateCommand.PlanPrompt);
            Assert.NotNull(policyCommand);
            Assert.Equal(currentPath, policyCommand!.WorkspacePath);
            Assert.Equal(["MIT", "Apache-2.0"], policyCommand.BannedLicenses);
            Assert.Equal(["Tailspin"], policyCommand.BannedOrganizations);
        });
    }
    ...
    private sealed class WorkspaceApiClientStub : IWorkspaceApiClient
    {
        ...
        public Func<UpdateWorkspacePolicyCommand, CancellationToken, Task<bool>>? OnUpdateWorkspacePolicyAsync { get; init; }
        public Func<UpdateWorkspaceCommand, CancellationToken, Task<WorkspaceMutationOutcome>>? OnUpdateWorkspaceAsync { get; init; }
        ...
        public Task<bool> UpdateWorkspacePolicyAsync(UpdateWorkspacePolicyCommand command, CancellationToken ct = default)
            => OnUpdateWorkspacePolicyAsync?.Invoke(command, ct) ?? throw new NotImplementedException();
        ...
        public Task<WorkspaceMutationOutcome> UpdateWorkspaceAsync(UpdateWorkspaceCommand command, CancellationToken ct = default)
            => OnUpdateWorkspaceAsync?.Invoke(command, ct) ?? throw new NotImplementedException();
    ...
    private sealed class AllowAllAuthorizationPolicyService : IAuthorizationPolicyService
    {
