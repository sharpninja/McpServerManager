...
public sealed class DirectorAuthorizationPolicyServiceTests
{
    ...
    [Fact]
    public void ViewerRole_CanViewReadOnlyTriageArea()
    {
        var policy = new DirectorAuthorizationPolicyService(new FakeRoleContext(roles: [McpRoles.Viewer]));
    ...
    [InlineData(McpActionKeys.MemoryList, McpRoles.Viewer)]
    [InlineData(McpActionKeys.MemoryGet, McpRoles.Viewer)]
    [InlineData(McpActionKeys.MemoryAdd, McpRoles.Viewer)]
    [InlineData(McpActionKeys.MemoryUpdate, McpRoles.Viewer)]
    [InlineData(McpActionKeys.MemoryRemove, McpRoles.Viewer)]
