...
internal sealed class TodoApiClientAdapter : ITodoApiClient
{
    ...
    public TodoApiClientAdapter(WebMcpContext context, ILogger<TodoApiClientAdapter>? logger = null)
    {
        _context = context;
    ...
    public async Task<ListTodosResult> ListTodosAsync(ListTodosQuery query, CancellationToken cancellationToken = default)
    {
        var response = await _context.UseActiveWorkspaceApiClientAsync(
                (client, ct) => client.Todo.QueryAsync(
    ...
    public async Task<TodoMutationOutcome> CreateTodoAsync(CreateTodoCommand command, CancellationToken cancellationToken = default)
    {
        try
        {
            var result = await _context.UseActiveWorkspaceApiClientAsync(
                    (client, ct) => client.Todo.CreateAsync(new TodoCreateRequest
                    {
                        Id = command.Id,
                        Title = command.Title,
    ...
    public async Task<TodoMutationOutcome> UpdateTodoAsync(UpdateTodoCommand command, CancellationToken cancellationToken = default)
    {
        ...
        {
            var result = await _context.UseActiveWorkspaceApiClientAsync(
                    (client, ct) => client.Todo.UpdateAsync(command.TodoId, new TodoUpdateRequest
                    {
                        Title = command.Title,
                        Section = command.Section,
                        Priority = command.Priority,
    ...
    public async Task<TodoMutationOutcome> DeleteTodoAsync(DeleteTodoCommand command, CancellationToken cancellationToken = default)
    {
        ...
        {
            var result = await _context.UseActiveWorkspaceApiClientAsync(
                    (client, ct) => client.Todo.DeleteAsync(command.TodoId, ct),
                    cancellationToken)
                .ConfigureAwait(true);
            return MapMutationOutcome(result);
        }
    ...
    public async Task<TodoMutationOutcome> MoveTodoAsync(MoveTodoCommand command, CancellationToken cancellationToken = default)
    {
        ...
        {
            var result = await _context.UseActiveWorkspaceApiClientAsync(
                    (client, ct) => client.Todo.MoveAsync(command.TodoId, new TodoMoveRequest
                    {
                        TargetWorkspacePath = command.TargetWorkspacePath,
                    }, ct),
                    cancellationToken)
                .ConfigureAwait(true);
            return MapMutationOutcome(result);
    ...
    {
        var result = await _context.UseActiveWorkspaceApiClientAsync(
                (client, ct) => client.Todo.AnalyzeRequirementsAsync(todoId, ct),
                cancellationToken)
            .ConfigureAwait(true);
    ...
    private IAsyncEnumerable<string> GetPromptStreamAsync(
        Func<McpServerClient, IAsyncEnumerable<string>> getStream,
        CancellationToken cancellationToken)
        => _context.StreamActiveWorkspaceApiClientAsync(
            (client, _) => getStream(client),
            cancellationToken);
