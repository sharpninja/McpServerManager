...
/// <summary>Round-trip between TODO item and YAML front matter + markdown (same format as VS Code extension).</summary>
public static class TodoMarkdown
{
    /// <summary>Returns a blank YAML front matter + markdown template for creating a new TODO item.</summary>
    ...
    /// <summary>Parse a new-todo YAML front matter + markdown template and extract title and priority (default: low).</summary>
    public static NewTodoFields ToNewTodoFields(string markdown)
    {
        if (markdown == null) throw new ArgumentNullException(nameof(markdown));
        var fields = new NewTodoFields();
        ...
        foreach (var line in fm)
        {
            ...
            switch (key)
            {
                case "PRIORITY" when !string.IsNullOrWhiteSpace(value): fields.Priority = value.ToLowerInvariant(); break;
            }
        }

        // Title from H1 in body
        foreach (var line in body)
        {
            if (line.StartsWith("# ", StringComparison.Ordinal))
            {
                fields.Title = line.Substring(2).Trim();
                break;
            }
        }
    ...
    public static string ToMarkdown(TodoFlatItem item)
    {
        ...
        // Body: H1 title, description, H2 sections
    ...
    public static TodoUpdateBody FromMarkdown(string markdown)
    {
        if (markdown == null) throw new ArgumentNullException(nameof(markdown));
        var updateBody = new TodoUpdateBody();
        var (fm, bodyLines) = SplitFrontMatter(markdown);

        // Parse all front matter keys generically; skip only 'id'
        ...
        foreach (var line in fm)
        {
            ...
            {
                AssignListField(updateBody, currentListKey, currentList);
                currentListKey = null;
                currentList = null;
            }
            ...
            // List start: empty value or []
            if (value == "" || value == "[]")
            {
                currentListKey = key;
                currentList = new List<string>();
                if (value == "[]") AssignListField(updateBody, key, currentList);
                continue;
            }
            ...
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                ...
                AssignListField(updateBody, key, items);
                continue;
            }

            AssignScalarField(updateBody, key, value);
        }
        ...
        // Parse body for H1 title, description, and H2 sections
        ...
        foreach (var line in bodyLines)
        {
            ...
            {
                updateBody.Title = trimmed.Substring(2).Trim();
            ...
            switch (currentSection)
            {
                ...
                case "implementation-tasks":
                    ...
                    if (taskMatch.Success)
                        tasks.Add(new TodoFlatTask { Task = taskMatch.Groups[2].Value, Done = taskMatch.Groups[1].Value.Equals("x", StringComparison.OrdinalIgnoreCase) });
                    else
                    {
                        var plainBullet = Regex.Match(trimmed, @"^-\s+(.+)$");
                        if (plainBullet.Success)
                            tasks.Add(new TodoFlatTask { Task = plainBullet.Groups[1].Value, Done = false });
                    }
                    break;
                default:
        ...
        if (description.Count > 0) updateBody.Description = description;
        if (technicalDetails.Count > 0) updateBody.TechnicalDetails = technicalDetails;
        if (tasks.Count > 0) updateBody.ImplementationTasks = tasks;
    ...
    private static void AssignScalarField(TodoUpdateBody body, string key, string value)
    {
        switch (key)
        {
    ...
    private static void AssignListField(TodoUpdateBody body, string key, List<string> items)
    {
        switch (key)
        {
