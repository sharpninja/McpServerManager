...
public sealed class McpTodoService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await _httpClient.GetFromJsonAsync<McpTodoQueryResult>(url, cancellationToken).ConfigureAwait(false)
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("/mcp/todo", request, cancellationToken).ConfigureAwait(false);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<McpTodoMutationResult>(cancellationToken: cancellationToken).ConfigureAwait(false)
    ...
    {
        var response = await _httpClient.PutAsJsonAsync($"/mcp/todo/{Uri.EscapeDataString(id)}", request, cancellationToken).ConfigureAwait(false);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<McpTodoMutationResult>(cancellationToken: cancellationToken).ConfigureAwait(false)
               ?? new McpTodoMutationResult();
    }
    ...
    {
        var response = await _httpClient.DeleteAsync($"/mcp/todo/{Uri.EscapeDataString(id)}", cancellationToken).ConfigureAwait(false);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<McpTodoMutationResult>(cancellationToken: cancellationToken).ConfigureAwait(false)
               ?? new McpTodoMutationResult();
    }
    ...
    {
        var response = await _httpClient.PostAsync($"/mcp/todo/{Uri.EscapeDataString(id)}/requirements", null, cancellationToken).ConfigureAwait(false);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<McpRequirementsAnalysisResult>(cancellationToken: cancellationToken).ConfigureAwait(false)
