...
internal static class DirectorCommands
{
    ...
    /// </summary>
    internal static Dictionary<string, string> ParseMarkerFields(string[] lines)
    {
    ...
    internal static bool VerifyMarkerSignature(Dictionary<string, string> fields, out string error)
    {
        ...
        fields.TryGetValue("agent_plugins.policy", out var agentPluginsPolicy);
        fields.TryGetValue("agent_plugins.contract_digest", out var agentPluginsContractDigest);
        var hasAgentPluginContract =
            !string.IsNullOrEmpty(agentPluginsPolicy) ||
            !string.IsNullOrEmpty(agentPluginsContractDigest);
        if (hasAgentPluginContract)
        {
            payload.Append("agentPlugins.policy=").Append((agentPluginsPolicy ?? string.Empty).ReplaceLineEndings("\n")).Append('\n');
            payload.Append("agentPlugins.contractDigest=").Append((agentPluginsContractDigest ?? string.Empty).ReplaceLineEndings("\n")).Append('\n');
        }
