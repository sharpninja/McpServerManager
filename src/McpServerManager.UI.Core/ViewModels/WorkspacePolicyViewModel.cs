...
public partial class WorkspacePolicyViewModel : ObservableObject
{
    ...
    public string WorkspacePath => !string.IsNullOrWhiteSpace(WorkspacePathOverride)
    ...
    /// <summary>Optional selected workspace path for host screens editing a non-active workspace.</summary>
    [ObservableProperty]
    private string? _workspacePathOverride;
    ...
    /// <summary>Seeds the policy editor from a loaded workspace detail.</summary>
    /// <param name="detail">Workspace detail containing the current policy lists.</param>
    public void LoadFromDetail(WorkspaceDetail detail)
    {
        ArgumentNullException.ThrowIfNull(detail);

        ReplaceCollection(BannedLicenses, detail.BannedLicenses);
        ReplaceCollection(BannedCountriesOfOrigin, detail.BannedCountriesOfOrigin);
        ReplaceCollection(BannedOrganizations, detail.BannedOrganizations);
        ReplaceCollection(BannedIndividuals, detail.BannedIndividuals);
        WorkspacePathOverride = detail.WorkspacePath;
        ErrorMessage = null;
        SaveSucceeded = false;
    }

    /// <summary>Clears all policy editor fields.</summary>
    public void ClearPolicy()
    {
        BannedLicenses.Clear();
        BannedCountriesOfOrigin.Clear();
        BannedOrganizations.Clear();
        BannedIndividuals.Clear();
    ...
    partial void OnWorkspacePathOverrideChanged(string? value) => OnPropertyChanged(nameof(WorkspacePath));
}
