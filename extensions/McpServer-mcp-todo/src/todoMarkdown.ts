 ...
 */
export function blankTodoTemplate(): string {
  return [
    '---',
    'id: NEW-TODO',
    'section: mvp-app',
...
export function todoToMarkdown(item: TodoFlatItem): string {
  ...
  if (item.done) fm.push('done: true');
  if (item.estimate) fm.push(`estimate: ${yamlScalar(item.estimate)}`);
  if (item.note) fm.push(`note: ${yamlScalar(item.note)}`);
  if (item.completedDate) fm.push(`completed: ${item.completedDate}`);
  if (item.doneSummary) fm.push(`done-summary: ${yamlScalar(item.doneSummary)}`);
  if (item.remaining) fm.push(`remaining: ${yamlScalar(item.remaining)}`);
...
/** Fields needed to create a new TODO via the CLI command. */
 ...
 * Priority defaults to 'low' if not specified.
 */
export function markdownToNewTodoFields(markdown: string): NewTodoFields {
 ...
 * Title comes from H1, technical details and implementation tasks from H2 sections.
 */
export function markdownToUpdateBody(markdown: string): TodoUpdateBody {
  const { frontMatter, body } = splitFrontMatter(markdown);
  const fm = parseFrontMatter(frontMatter);
  const sections = parseBody(body);

  // Copy all front matter fields except id
  const updateBody: TodoUpdateBody = {};
  if (fm.priority !== undefined) updateBody.priority = fm.priority;
  if (fm.section !== undefined) updateBody.section = fm.section;
  if (fm.done !== undefined) updateBody.done = fm.done;
  if (fm.estimate !== undefined) updateBody.estimate = fm.estimate;
  if (fm.note !== undefined) updateBody.note = fm.note;
  if (fm.completedDate !== undefined) updateBody.completedDate = fm.completedDate;
  if (fm.doneSummary !== undefined) updateBody.doneSummary = fm.doneSummary;
  if (fm.remaining !== undefined) updateBody.remaining = fm.remaining;
  if (fm.dependsOn !== undefined) updateBody.dependsOn = fm.dependsOn;
  if (fm.functionalRequirements !== undefined) updateBody.functionalRequirements = fm.functionalRequirements;
  if (fm.technicalRequirements !== undefined) updateBody.technicalRequirements = fm.technicalRequirements;

  // Body sections override front matter equivalents
  updateBody.title = sections.title ?? fm.title;
  if (sections.description.length) updateBody.description = sections.description;
  if (sections.technicalDetails.length) updateBody.technicalDetails = sections.technicalDetails;
  else if (fm.technicalDetails !== undefined) updateBody.technicalDetails = fm.technicalDetails;
  if (sections.implementationTasks.length) updateBody.implementationTasks = sections.implementationTasks;
  else if (fm.implementationTasks !== undefined) updateBody.implementationTasks = fm.implementationTasks;
...
function splitFrontMatter(doc: string): { frontMatter: string[]; body: string[] } {
  ...
  while (i < lines.length) {
      ...
      while (i < lines.length && /^\s+/.test(lines[i]!)) {
        ...
        if (taskMatch) {
          ...
          if (i + 1 < lines.length) {
            ...
            if (doneMatch) { done = doneMatch[1].toLowerCase() === 'true'; i++; }
          }
    ...
    if (isListStart && (key === 'depends-on' || key === 'technical-details' || key === 'functional-requirements' || key === 'technical-requirements')) {
      ...
      while (i < lines.length && /^\s+-\s/.test(lines[i]!)) {
        ...
        if (itemMatch) arr.push(unquote(itemMatch[1].trim()));
        i++;
      }
      if (key === 'depends-on') fm.dependsOn = arr;
      else if (key === 'functional-requirements') fm.functionalRequirements = arr;
      else if (key === 'technical-requirements') fm.technicalRequirements = arr;
      else fm.technicalDetails = arr;
      continue;
    }
    ...
    if (value.startsWith('[') && value.endsWith(']')) {
      ...
      else if (key === 'technical-details') fm.technicalDetails = arr;
      i++;
      continue;
    }
    ...
    switch (key) {
      case 'id': fm.id = v; break;
      case 'title': fm.title = v; break;
      case 'section': fm.section = v; break;
      case 'priority': fm.priority = v.toLowerCase(); break;
      case 'done': fm.done = /^(true|1|yes)$/i.test(v); break;
      case 'estimate': fm.estimate = v; break;
      case 'note': fm.note = v; break;
      case 'completed': fm.completedDate = v; break;
      case 'done-summary': fm.doneSummary = v; break;
      case 'remaining': fm.remaining = v; break;
    }
    i++;
  }
  return fm;
...
function parseBody(lines: string[]): ParsedBody {
  ...
  let currentSection: 'description' | 'technical-details' | 'implementation-tasks' = 'description';
  ...
  for (const line of lines) {
    ...
    if (h2) {
      ...
      if (heading.includes('technical')) currentSection = 'technical-details';
      else if (heading.includes('implementation') || heading.includes('task')) currentSection = 'implementation-tasks';
